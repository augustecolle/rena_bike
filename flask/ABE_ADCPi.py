...
class ADCPi:
    ...
    __address = 0x6e  # default address for adc 1 on adc pi and delta-sigma pi
    __config1 = 0x9C  # PGAx1, 18 bit, continuous conversion, channel 1 - Het standaard mask voor de instellingen
    ...
    def __init__(self, bus, address, rate=18):
        self._bus = bus
	...
	global R_s
        ...
        if (self.__signbit):
            ...
            voltage = float((raw * (self.__lsb / self.__pga)) * (33+1.15)/1.15)
            return float(voltage)
	elif (channel ==2):
            voltage = float((raw * (self.__lsb / self.__pga)) * 1.0/(R_L * 200e-6 * R_s))
            return float(voltage)
