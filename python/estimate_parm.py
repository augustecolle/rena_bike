...
def main():
    ...
    import matplotlib.pyplot as plt
    speed = []
    myDB = MySQLdb.connect(host="192.168.1.18",port=3306,user="auguste",passwd="renasolutions",db="eBike")
    cHandler = myDB.cursor()
    cHandler.execute("SELECT gps_speed FROM measurements")
    results = cHandler.fetchall()
    for item in results:
        speed.append(float(item[0]))
    plt.plot(speed)
    plt.show()
